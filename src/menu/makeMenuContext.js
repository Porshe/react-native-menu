...
module.exports = (React, ReactNative, { constants, model, styles }) => {
    ...
    const makeOptions = (options, { top, right, bottom }) => {
        const {
        ...
        return (
            <AnimatedOptionsContainer
                style={[styles.optionsContainer, optionsContainerStyle, { top, right, bottom }]}
            >
   ...
   */
    const MenuContext = createReactClass({
        ...
        _makeAndPositionOptions(name, menuMeasurements) {
            ...
            const { w: ownWidth, px: ownPX, py: ownPY, h: ownHeight } = this._ownMeasurements
            let optionsTop = menuPY - ownPY
            ...
            if(this.props.children && (ownHeight - optionsTop) / this.props.children.length < 40){
                style.bottom = 5;
            }else{
                style.top = optionsTop;
            }

            return makeOptions(options, style)
        },
